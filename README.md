...
- `@experiments/ui`: a stub React component library shared by both `web` and `docs` applications
- `@experiments/eslint-config`: `eslint` configurations (includes `eslint-config-next` and `eslint-config-prettier`)
- `@experiments/typescript-config`: `tsconfig.json`s used throughout the monorepo
